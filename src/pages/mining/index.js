    ...
    setAvailableBalance((prevBalance) => {
      ...
      if (newBalance >= coinsPerLevel) {
        setCurrentLevel((prevLevel) => {
          ...
          const newTier =
            levelThresholds.find((threshold) => newLevel <= threshold.maxLevel)
              ?.name || "Bronze";
    ...
    >
      ...
      <div className="bg-gradient-to-b from-[#74561d] via-[#303c20] to-[#303c20] py-6 rounded-t-3xl shadow-lg shadow-[#9b8255]">
        <div className="flex justify-center space-x-4">
          {/* 3 details button */}
          <DetailsButton
            className="text-center shadow-lg bg-gray-100 p-3  rounded-md"
            heading={"Earn per Tap"}
            IsIcon={true}
            value={`+${perTapValue}`}
          />
          <DetailsButton
            className="text-center shadow-lg bg-gray-100 p-4 rounded-md"
        ...
        <div className="flex text-center justify-center mt-4">
          <span className="flex items-center">
            <img className="w-28" src={CountingCoin} alt="leon-counting-coin" />
            <p className="text-white text-3xl mr-4 font-semibold">
              {availableBalance}
            </p>
          </span>
        </div>
            ...
            <p className="text-white text-xs">
              Level {currentLevel}/
              {
                levelThresholds.find(
                  (threshold) => threshold.name === currentTier
                )?.maxLevel
              }
            </p>
          </div>
